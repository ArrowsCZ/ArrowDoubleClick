...
public static class ArDoubleClick
{
    ...
    static ArDoubleClick()
    {
        if (!Directory.Exists(Path.GetDirectoryName(FilePath)))
            Directory.CreateDirectory(Path.GetDirectoryName(FilePath)!); // 在指定的路径下创建一个新的文件夹。
        if (!File.Exists(FilePath))
        {
            File.WriteAllBytes(FilePath, ResourceTools.ClickSettings); // 在指定的路径下写入一个新的配置。
#if Debug
            Acaop.ShowAlertDialog($"配置文件缺失，已从资源中写入文件:{FilePath}！");
#endif
        }
    ...
    private static readonly string FilePath = Path.Combine(
        Path.GetDirectoryName(GetAssemblyPath(true)) ?? string.Empty,
        "Settings",
        "ClickSettings.xml"
    );
